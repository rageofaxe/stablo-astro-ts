<script lang="ts">
  ...
  let exceptions: Array<keyof Knife> = ["isMetricNumbers"];
...
<div class="flex">
  <div class="flex flex-col w-1/4 cursor-pointer">
    {#each keys as key}
      <div
        class="p-4 font-bold border-b-black border-b max-h-20 min-h-20 overflow-hidden">
        {key}
      </div>
    {/each}
  </div>
  {#each Object.values(knives) as knife}
    <div
      class="flex flex-col w-1/4 cursor-pointer hover:bg-sky-100 hover:w-3/4 transition-all transition-duration: 300ms">
      {#each keys as key}
